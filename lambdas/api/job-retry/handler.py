"""Job retry Lambda (Spec/09 §4 L2). Re-runs a failed execution. Admins/Operations."""
...
def handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    try:
        ...
        boto3.resource("dynamodb").Table(os.environ["JOBS_TABLE"]).update_item(
            ...
            ExpressionAttributeValues={":s": "retrying"},
        )
        return _resp({"job_id": job_id, "status": "retrying"}, 202)
    except Exception:
        logger.exception("job-retry failed")
        raise
